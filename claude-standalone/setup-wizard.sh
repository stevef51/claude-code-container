...
#
# Each sandbox gets a folder with helper scripts:
#
#   ~/.claude-sandbox/sandboxes/<name>/start     — launch (auto-resumes)
#   ~/.claude-sandbox/sandboxes/<name>/clear     — clear conversation & start fresh
#   ~/.claude-sandbox/sandboxes/<name>/reset     — wipe repo & reclone
#   ~/.claude-sandbox/sandboxes/<name>/upgrade   — rebuild image (picks up new skills)
#
...
#
# Start (auto-resumes if previous conversation exists):
#   docker compose -f ${compose_file} run --rm claude
#
# Clear conversation state and start fresh:
#   CLEAR_STATE=1 docker compose -f ${compose_file} run --rm claude
#
...
SCRIPT
  ...
  # clear (wipe conversation state + start fresh)
  cat > "$sandbox_dir/clear" << 'SCRIPT'
#!/usr/bin/env bash
set -euo pipefail
DIR="$(cd "$(dirname "$0")" && pwd)"
exec CLEAR_STATE=1 docker compose -f "$DIR/docker-compose.yaml" run --rm claude
SCRIPT

  # reset (wipe repo volume and reclone)
...
SCRIPT
  ...
  # upgrade (rebuild image — new skills picked up on next start)
  cat > "$sandbox_dir/upgrade" << 'SCRIPT'
#!/usr/bin/env bash
set -euo pipefail
CONFIG="$HOME/.claude-sandbox/config"
if [[ ! -f "$CONFIG" ]]; then
  echo "Error: Config not found at $CONFIG" >&2
  exit 1
fi
source "$CONFIG"
echo "Rebuilding image '${IMAGE_NAME}' from ${CONTAINER_BUILD_DIR}..."
cd "$CONTAINER_BUILD_DIR"
docker build -t "$IMAGE_NAME" .
echo ""
echo "Done. New skills and tools will be active on next start."
SCRIPT
...
SCRIPT
  ...
  chmod +x "$sandbox_dir"/{start,clear,reset,stop,upgrade,destroy}
}
...
echo ""
echo -e "  ${CYAN}start${NC}    — Launch Claude (auto-resumes previous conversation)"
echo -e "  ${CYAN}clear${NC}    — Clear conversation state and start fresh"
echo -e "  ${CYAN}reset${NC}    — Wipe repo and reclone"
echo -e "  ${CYAN}upgrade${NC}  — Rebuild Docker image (picks up new skills/tools)"
